...
class Node:
    ...
    def _check_inter_affinity(self):
        ...
        for app in self.applicationList:
            ...
            tput_change = (-sum(exceed_card) * 0.3) + (sum(within_card) * 0.1)
            """
    ...
    def _update_tput(self):
        ...
        for app in self.applicationList:
            ...
            app.throughput = min(app.bwThroughput, app.memBwThroughput, app.cacheThroughput) * app.affinityThroughput / app.baseThroughput
    ...
    def tput_breakdown(self):
        """
        return the breakdown throughput of all the containers residing in this node
        """
        import numpy as np
        tput_breakdown = np.zeros([1, self.env.NUM_APPS])
        for app in self.applicationList:
            tput_breakdown[0,app.applicationName] = app.throughput
        return tput_breakdown
    ...
    def individual_tput(self):
        ...
        import numpy as np
        # app_reduce
        tput = np.zeros([1,self.env.NUM_APPS])
        for app in self.applicationList:
            tput[0,app.applicationName] = app.throughput * app.numberofContainers
        return tput